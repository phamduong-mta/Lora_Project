...
/* USER CODE BEGIN 0 */
extern char uartBuff[UART_BUFF_SIZE];
...
extern volatile uint8_t txBuffer[TXBUFF_SIZE];
extern volatile uint8_t txBuffer_encypt[TXBUFF_SIZE];
extern volatile uint32_t RF_FREQUENCY;
...
void eventRxDone(pingPongFSM_t *const fsm) {
	...
	case STATE_MASTER:
		...
		case SSTATE_RX:
			...
			//printf("Node id hien tai la: %s\r\n", Node_id);
			if (strncmp(rxBuffer_decypt, Node_id, 6) == 0) {
				//printf("Receive data: %s\r\n", rxBuffer_decypt);
				uint8_t buff_done[256];
				strcpy(buff_done, &rxBuffer_decypt[6]);
				Uart_Indicate();
//				HAL_UART_Transmit(&huart1, (uint8_t*) rxBuffer_decypt,
//						strlen(rxBuffer_decypt), 1000);
